...
from src.pipeline.harvester.authentication import AuthenticationError
    ...
    def harvest_all(self):
        ...
        if self.console:
            ...
            if self.failed_slugs:
                msg += f"\n[bold red]Failures: {len(self.failed_slugs)}[/bold red]"
            self.console.print(Panel(msg, title="Success", border_style="green"))
            
        return self.failed_slugs

    def retry_failed(self, failed_items: List[Dict]):
        """
        Retries only the failed items using the same sequential logic.
        """
        if not failed_items:
            return
        ...
        with Progress(
            ...
            for item in failed_items:
                ...
                try:
                    progress.update(main_task, description=f"[yellow]Retrying: {slug}")
